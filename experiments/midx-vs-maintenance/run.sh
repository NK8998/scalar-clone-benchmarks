...
drop_caches() {
    sync
    # Prefer a sanctioned NOPASSWD helper if the box provides one; the inline
    # `sh -c` form below is not covered by a command-specific sudoers rule.
...
grep -q -- '--\[no-\]prefetch-cache-server-url' < <("$PREFIX/bin/scalar" clone -h 2>&1) \
